...
export default function OtpValidationModal({ order, isOpen, onClose, onSuccess }: OtpValidationModalProps) {
  ...
  const handleValidate = (e: React.FormEvent) => {
    ...
    const res = sugubaStore.verifyDeliveryOtp(
      order.id,
      otpInput.trim(),
        ...
        <div className="p-5 space-y-4">
          ...
          {isSuccess ? (
            ...
            <form onSubmit={handleValidate} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-bold text-slate-700">
                    Demandez au client son Code Secret (4 chiffres) *
                  </label>
                  {(order.failedOtpAttempts || 0) > 0 && (
                    <span className="text-[10px] font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-md border border-rose-200">
                      Tentative {(order.failedOtpAttempts || 0)} / 3
                    </span>
                  )}
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    maxLength={4}
                    disabled={(order.failedOtpAttempts || 0) >= 3}
                    placeholder="Ex: 5832"
                    value={otpInput}
              ...
              {/* Simulation hint for testing */}
              <div className="bg-slate-100 p-2.5 rounded-xl text-center text-[10px] text-slate-500 font-mono">
                [Code démo de cette commande : <strong className="text-slate-800">{order.deliveryOtp}</strong>]
              </div>
              ...
              {(order.failedOtpAttempts || 0) >= 3 ? (
                ...
                >
                  ...
                  <span>Valider le Code & Encaisser</span>
                </button>
              )}
