...
function LoginPageContent() {
  ...
  // Déjà connecté (2026-09-24) : on ne redemande pas la connexion. Une
  // personne revenue sur /login alors que sa session Google était posée
  // (onglet resté ouvert, bouton retour…) recommençait tout le parcours.
  // Pas de renvoi automatique quand le middleware signale un espace refusé
  // (?denied=) : le compte connecté n'y a pas accès, il faut en changer.
  const [dejaConnecte, setDejaConnecte] = useState(false);
  useEffect(() => {
    if (deniedRole) return;
    rafraichirIdentite({ forcer: true }).then((moi) => {
      if (!moi?.authenticated || !moi.role) return;
      setDejaConnecte(true);
      const incomplet = moi.role !== 'admin' && (moi.phone || '').includes('@');
      const espace = ESPACE_PAR_ROLE[moi.role] || '/';
      window.location.replace(incomplet ? '/register/complete' : (prendreApresConnexion() || espace));
    });
  }, [deniedRole]);
    ...
    <div className="min-h-screen flex flex-col bg-[#f5f8f5]">
      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-sm space-y-4">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-7 border border-gray-100 shadow-float">
            ...
            {dejaConnecte && (
              <div role="status" className="mb-4 p-3 bg-suguba-menthe rounded-2xl text-xs font-semibold text-suguba-profond text-center">
                Vous êtes déjà connecté — ouverture de votre espace…
              </div>
            )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                {quickRoles.map(({ role, label, icon: Icon, color, dest }) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-bold leading-none block">{label}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="text-center pb-6 px-4">
        <p className="text-xs text-gray-300">
          ...
          et la{' '}
          <Link href="/legal/privacy" className="text-suguba-brand-dark hover:underline">
